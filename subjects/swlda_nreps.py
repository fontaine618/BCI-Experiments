...
dir_chains = "/home/simon/Documents/BCI/experiments/subjects/chains/"
...
# save bmat
np.save(dir_chains + f"K{subject}_trnreps{trn_reps}.swlda", Bmat.cpu().numpy())
# -----------------------------------------------------------------------------
...
# get predictions
trn_pred, trn_agg_pred, trn_cum_pred = swlda_predict(Bmat.cpu(), trnX, trnstim, eeg.keyboard)
# get true
...
trndf["acc"] = (trndf["char_pred"] == trndf["char_true"]).astype(float)
trndf = trndf.groupby("repetition").agg({"hamming": "mean", "acc": "mean"})
trndf["dataset"] = "testing"
trndf["method"] = "swLDA"
trndf.reset_index(inplace=True)
trndf["subject"] = subject
trndf["training_reps"] = trn_reps
trndf["repetition"] = trndf["repetition"] - trn_reps
out.append(trndf)
# -----------------------------------------------------------------------------
...
df = pd.concat(out)
os.makedirs(dir_results, exist_ok=True)
df.to_csv(dir_results + f"K{subject}_trnreps{trn_reps}.swlda")
print(df)
# -----------------------------------------------------------------------------
