...
# =============================================================================
# MAKE PLOTS
subjects = [114, 117, 171, 183, 146, 151]
groups = ["Control", "Control", "Control", "Control", "ALS", "ALS"]
yyrange = (-15, 25)
plot_name = "ALL"
ncols = len(subjects)
...
fig, axes = plt.subplots(
    ...
    gridspec_kw={
        ...
        "height_ratios": [1.5, 1]*K + [0.5]
    },
    sharex="row",
    sharey="row"
)
for col, (subject, group) in enumerate(zip(subjects, groups)):
    # LOAD POSTERIOR
    subject = str(subject)
    ...
    for k in range(K):
        ...
        blank_canvas(ax)
        if k == 0:
            ax.set_title(f"Subject {subject} ({group})", fontweight="bold")
        ...
        if col == 0:
            ax.set_ylabel("Mean process")

# ADD LEGEND
gs = axes[K*2, 0].get_gridspec()
for ax in axes[K*2, :]:
    ax.remove()
axlegend = fig.add_subplot(gs[K*2, :], frameon=False)
axlegend.axis("off")
