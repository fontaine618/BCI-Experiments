...
seeds = range(10)
train_reps = [7]
sample_mean = ["arithmetic", "geometric", "harmonic", "psis"]
which_first = ["sequence", "sample"]
experiment = list(it.product(train_reps, seeds, sample_mean, which_first))
...
for train_reps, seed, sample_mean, which_first in experiment:
...
results_df = results_df.groupby(["which_first", "sample_mean", "repetition"]).agg({
    "hamming": ["mean", "std"],
    "acc": ["mean", "std"],
...
# hue is sample_mean
# error bars are std
results_df["Operation order"] = (results_df["which_first"] + " first").str.capitalize()
results_df["Mean across samples"] = results_df["sample_mean"].str.capitalize()
...
g = sns.lineplot(
    ...
    x="repetition",
    y=("bce", "mean"),
    style="Operation order",
    hue="Mean across samples"
)
plt.xlabel("Testing repetition")
plt.ylabel("Binary cross-entropy")
# change legend subtitles
plt.tight_layout()
plt.savefig(dir_figures + f"K{subject}_trn{train_reps}_bce_aggregation.pdf")
