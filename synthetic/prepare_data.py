...
heterogeneities = 3.
shrinkage_factor = (2., 2.)
scaling_activation = "exponential"
seed = 0
...
smgp_scaling_mixing = torch.vstack([
	(torch.sigmoid((t-1-5*i))*torch.sigmoid((stimulus_window-t-15+5*i))).reshape(1, -1)
...
smgp_scaling_target = torch.vstack([
	torch.sin(2 * torch.pi * (t - 5*i) / 30)
	for i in range(latent_dim)
])
smgp_scaling_target = smgp_scaling_target * \
						 (torch.sigmoid((t-1)*2)*torch.sigmoid((stimulus_window-t-10)/3)).reshape(1, -1)
smgp_scaling_target = smgp_scaling_target * 0.5
smgp_scaling_target *= smgp_scaling_mixing
...
smgp_scaling_nontarget = smgp_scaling_nontarget * 0.2
smgp_scaling_nontarget *= smgp_scaling_mixing
